...
const alertVariants = cva(
	"group/alert relative grid w-full gap-0.5 rounded-lg border px-4 py-3 text-left text-sm has-data-[slot=alert-action]:relative has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-2.5 has-data-[slot=alert-action]:pr-18 *:[svg:not([class*='size-'])]:size-4 *:[svg]:row-span-2 *:[svg]:translate-y-0.5 *:[svg]:text-current",
	{
		variants: {
			variant: {
				default:
					"dark:!bg-gray-300/20 dark:!text-gray-300 border-gray-500/40 bg-gray-500/10 text-gray-500 *:data-[slot=alert-description]:text-gray-500/90 dark:border-gray-300/50 dark:*:data-[slot=alert-description]:text-gray-300/90",
				destructive:
					"dark:!bg-destructive/20 border-destructive/40 bg-destructive/10 text-destructive *:data-[slot=alert-description]:text-destructive/90 dark:border-destructive/50 *:[svg]:text-current",
			},
		},
		defaultVariants: {
			variant: "default",
